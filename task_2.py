...
def local_beam_search(k_head, queen_nums):
    init_state = []
    steps = 0
    ...
    while True:
        ...
        # print(all_next_state[:8])
        all_next_state = sorted(all_next_state, key = lambda x: x[0])
        # print(all_next_state[:8])
        for item in all_next_state[:queen_nums]:
            if item[0] == 0:
                return item[0],item[1]
        init_state = [all_next_state[:queen_nums][i][1] for i in range(queen_nums)]
        if steps <= 50:
            steps += 1
        else:
            print(all_next_state[0][0],all_next_state[0][1])
            return None, None

def random_restart_algorithm(k_head,queen_nums):
    min_score = None
    while True:
        min_score, queen_pos = local_beam_search(k_head,queen_nums)
        if min_score != 0:
            queen_pos = []
            continue
        else:
...
def main():
    k = 8
    queen_nums = 8
    min_score, queen_pos = random_restart_algorithm(k,queen_nums)
    board = np.zeros((queen_nums,queen_nums),dtype=int)
